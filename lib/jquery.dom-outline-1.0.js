 ...
 */
/**
 * This version has been slightly modified to stop event propagation
 */
var DomOutline = function (options) {
  ...
  function createOutlineElements() {
    self.elements.label = jQueryPPG('<div></div>').addClass(self.opts.namespace + '_label').appendTo('body');
    self.elements.top = jQueryPPG('<div></div>').addClass(self.opts.namespace).appendTo('body');
    self.elements.bottom = jQueryPPG('<div></div>').addClass(self.opts.namespace).appendTo('body');
    self.elements.left = jQueryPPG('<div></div>').addClass(self.opts.namespace).appendTo('body');
    self.elements.right = jQueryPPG('<div></div>').addClass(self.opts.namespace).appendTo('body');
  }

  function removeOutlineElements() {
    jQueryPPG.each(self.elements, function(name, element) {
      element.remove();
    });
  }
  ...
  function compileLabelText(element, width, height) {
    ...
    if (element.className) {
      label += ('.' + jQueryPPG.trim(element.className).replace(/ /g, '.')).replace(/\.\.+/g, '.');
    }
  ...
  function getScrollTop() {
    if (!self.elements.window) {
      self.elements.window = jQueryPPG(window);
    }
  ...
  function updateOutlinePosition(e) {
    ...
    if (self.opts.filter) {
      if (!jQueryPPG(e.target).is(self.opts.filter)) {
        return;
      }
    }
  ...
  pub.start = function () {
    ...
    if (self.active !== true) {
      ...
      jQueryPPG('body').on('mousemove.' + self.opts.namespace, updateOutlinePosition);
      jQueryPPG('body').on('keyup.' + self.opts.namespace, stopOnEscape);
      if (self.opts.onClick) {
        setTimeout(function () {
          jQueryPPG('body').on('click.' + self.opts.namespace, function(e){
            if (self.opts.filter) {
              if (!jQueryPPG(e.target).is(self.opts.filter)) {
                return false;
              }
  ...
  pub.stop = function () {
    ...
    jQueryPPG('body').off('mousemove.' + self.opts.namespace)
